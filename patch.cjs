...
const startPattern = "// 3. Facteur statut titulaire & pénalité derniers matchs";
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  if (card.scores?.xG && card.scores.xG > 0) advancedStatsBonus += Math.min(1.8, card.scores.xG * 1.1);
  if (card.scores?.xA && card.scores.xA > 0) advancedStatsBonus += Math.min(1.8, card.scores.xA * 1.1);

  // Dynamique Collective (Team Form) - Bonus contextuel 
  ...
  console.log('Successfully updated optimizer.ts');
} else {
